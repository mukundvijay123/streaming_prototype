...
public class Demo5{
    ...
    public static void main(String[] args){
        ...
        final List<Field> ordersFields = List.of(
            ...
            new Field("amount", FieldType.nullable(new ArrowType.FloatingPoint(FloatingPointPrecision.DOUBLE)), null) // timestamp as long
        );
        ...
        org.apache.beam.sdk.schemas.Schema ordersBeamSchema = null;
        try{
            ordersBeamSchema  = arrowIOUtils.ArrowSchemaConverter(ordersSchema);
            System.out.println(ordersBeamSchema);
        }catch(Exception e){
        ...
        //PCollection<Row> result=collection.apply(SqlTransform.query("SELECT order_id,amount FROM orders WHERE amount >500"));
    ...
    public class OrdersFaker implements Runnable {
        ...
        private VectorSchemaRoot createOrdersTable(Schema schema, BufferAllocator allocator) {
            ...
            //eventTime.setSafe(0,((int)TimestampValue));
            root.setRowCount(1);
            return root;
        }
